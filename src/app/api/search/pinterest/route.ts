import { NextRequest, NextResponse } from 'next/server';
import axios from 'axios';
import * as cheerio from 'cheerio';

const pin_headers = {
  "sec-ch-ua": '" Not A;Brand";v="99", "Chromium";v="90", "Google Chrome";v="90"',
  "sec-ch-ua-mobile": "?0",
  "upgrade-insecure-requests": "1",
  "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/90.0.4430.212 Safari/537.36",
  "accept": "text/html",
  "cookie": 'csrftoken=ebe0be3a93cea6072be18633add953a2; _b="AVezvd6F4UtE24FUsA6INxipyZZDoSpyCc5vaJK4QDYXmExosVEc4h6WkiKhlVtQ430="; cm_sub=denied; fba=True; _ga=GA1.2.862909259.1620474446; g_state={"i_l":0}; _auth=1; _pinterest_sess=; _pinterest_cm=""; _routing_id="595f24cd-7f4c-4495-aa67-37212d099cd8"; sessionFunnelEventLogged=1',
};

async function pinterest(query: string, limit: number = 20) {
  if (!query) throw new Error("Query is required");

  const url = "https://id.pinterest.com/search/pins/?q=" + encodeURIComponent(query);

  const res = await axios.get(url, {
    timeout: 30000,
    headers: pin_headers,
  });

  const $ = cheerio.load(res.data);
  const images = new Set<string>();

  $("script").each((_, el) => {
    const text = $(el).html();
    if (!text || !text.includes("i.pinimg.com")) return;

    const matches = text.match(/https:\/\/i\.pinimg\.com\/[^"'\\]+/g);
    if (!matches) return;

    matches.forEach((img) => {
      const cleanImg = img.replace(/\\u002F/g, '/');
      if (cleanImg.includes('_RS/') || cleanImg.includes('x75') || cleanImg.includes('x30') || cleanImg.includes('30x') || cleanImg.includes('75x')) return;
      
      const originalUrl = cleanImg.replace(/\/(236x|474x|564x|736x|140x140|280x280)\//, "/originals/");
      if (originalUrl.includes('/originals/') || originalUrl.includes('/736x/') || originalUrl.includes('/564x/')) {
        images.add(originalUrl);
      }
    });
  });

  const result = [...images].slice(0, limit);

  return {
    status: true,
    author: "Wanyzx",
    query,
    total: result.length,
    result,
  };
}

export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url);
    const query = searchParams.get('q') || searchParams.get('query');
    const limitParam = searchParams.get('limit') || searchParams.get('count') || '10';
    const limit = Math.min(Math.max(parseInt(limitParam) || 10, 1), 50);

    if (!query) {
      return NextResponse.json({
        success: false,
        error: 'Query parameter (q) is required',
        usage: {
          endpoint: '/api/search/pinterest',
          params: {
            q: 'Search query (required)',
            limit: 'Number of images to return (1-50, default: 10)',
          },
          example: '/api/search/pinterest?q=anime&limit=5',
        },
      }, { status: 400 });
    }

    const data = await pinterest(query, limit);

    return NextResponse.json({
      author: "Wanyzx",
      success: true,
      query,
      limit,
      total: data.total,
      data: data.result,
    });
  } catch (error) {
    console.error('Pinterest search error:', error);
    return NextResponse.json({
      success: false,
      error: error instanceof Error ? error.message : 'Failed to search Pinterest',
    }, { status: 500 });
  }
}
