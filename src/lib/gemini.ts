...
export async function gemini({ message, instruction = '', sessionId = null }: GeminiOptions) {
    try {
        ...
        if (!match || match.length < 4) {
            throw new Error('Gemini response format changed or invalid. Please use a different model.');
        }
